...
_BUTTON_COLORS = ("#16a34a", "#dc2626")  # first button green (approve-ish), rest red (reject-ish)


def _render_buttons(buttons: list[tuple[str, str]] | None) -> str:
    if not buttons:
        return ""
    links = "".join(
        f'<a href="{html.escape(url)}" style="display:inline-block;margin:6px 10px 6px 0;'
        f'padding:10px 22px;border-radius:6px;text-decoration:none;font-weight:bold;'
        f'background:{_BUTTON_COLORS[min(i, len(_BUTTON_COLORS) - 1)]};color:#ffffff;">'
        f'{html.escape(label)}</a>'
        for i, (label, url) in enumerate(buttons)
    )
    return f'<div style="margin-top:20px;">{links}</div>'


def _build_message(subject: str, message: str, buttons: list[tuple[str, str]] | None = None) -> MIMEMultipart:
    msg = MIMEMultipart()
    msg["Subject"] = subject
...
def send_notification_emails_sequential(
    recipients: list[tuple[str, str]], message: str, buttons: list[tuple[str, str]] | None = None,
) -> list[tuple[str, str]]:
    ...
    with smtplib.SMTP(settings.smtp_host, settings.smtp_port, timeout=15) as server:
        ...
        for to_email, subject in recipients:
            try:
                msg = _build_message(subject, message, buttons)
                msg["To"] = to_email
