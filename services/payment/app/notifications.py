...
notify_refund_processed() is the resident-facing counterpart: it resolves
the single resident who owns a payment_transaction (rather than the event's
organizer/permission grantees) and notifies them once their refund has
actually been paid out.

Split in two so the outbound HTTP fan-out never happens while a pooled DB
connection is held open (max_size=10 — see database.py):
  - resolve_and_record() / notify_refund_processed() run inside the
    caller's existing `async with pool.acquire() as conn:` block.
...
async def notify_refund_processed(
    ...
    row = await conn.fetchrow(
        ...
        "       pt.amount, pt.currency, e.title AS event_title, u.phone, "
    ...
    message = (
        ...
        f"\"{row['event_title']}\" has been processed by {actor_name}."
    )
    await conn.execute(
        "INSERT INTO notification (user_id, event_id, type, title, message, related_id) "
    ...
    return [{"phone": row["phone"]}], message
