      ...
      <Box sx={{ p: 2.5 }}>
        <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', mb: 1.5 }}>
          <Box sx={{ display: 'flex', gap: 1.5, alignItems: 'center' }}>
            <Box sx={{ width: 40, height: 40, borderRadius: 2, bgcolor: `${ticket.color}22`, display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 20 }}>
              {ticket.emoji}
            </Box>
            <Box>
              <Typography fontWeight={700} fontSize={15}>{ticket.event}</Typography>
        ...
        <Stack spacing={0.5} sx={{ mb: 2 }}>
          <Box sx={{ display: 'flex', gap: 1, alignItems: 'center', color: 'text.secondary' }}>
            ...
            <Typography fontSize={13} color="text.secondary">{ticket.date}</Typography>
          </Box>
          <Box sx={{ display: 'flex', gap: 1, alignItems: 'center', color: 'text.secondary' }}>
            <LocationOnIcon sx={{ fontSize: 13 }} />
            <Typography fontSize={13} color="text.secondary">{ticket.venue}</Typography>
          </Box>
        </Stack>
    ...
    <Box component="main" sx={{ bgcolor: '#f8fafc', minHeight: 'calc(100vh - 64px)', py: 4 }}>
      <Container maxWidth="md">
        ...
        <Box sx={{ display: 'flex', alignItems: 'center', gap: 2, mb: 3 }}>
          <ConfirmationNumberIcon sx={{ color: '#6366f1', fontSize: 30 }} />
          <Box>
            <Typography variant="h4" fontWeight={800} color="#0f172a">My Tickets</Typography>
        ...
        <Box sx={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: 2, mb: 3 }}>
          {[
            ...
            <Paper key={label} variant="outlined" sx={{ p: 2, borderLeft: `4px solid ${color}`, borderRadius: 2 }}>
        ...
        <Stack spacing={2}>
          {displayed.length === 0 ? (
            <Box sx={{ textAlign: 'center', py: 8 }}>
              ...
              <Typography mt={1} color="text.secondary">No tickets here yet.</Typography>
            </Box>
          ) : (
            displayed.map(t => (
